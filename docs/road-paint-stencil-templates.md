...
Road marking features now support:

- `etch` / existing visual marking export
- `paintStencil` / physical inverse template export

For stencil mode, road asset download creates a ZIP with the normal road deck SVG plus `road-paint-stencils/` sheets grouped by the selected real stock material ID. The road deck does not also engrave that marking; the physical stencil is the manufacturing output.
