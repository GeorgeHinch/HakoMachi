...
const LegacyHakoImportAndHealing = {
...
function legacyWallFeaturesForFace(cfg, face) {
  return LegacyHakoImportAndHealing.readFaceFeatures(cfg, face);
}

function syncLegacyWallFeatureBuckets(cfg, face, features) {
...
function repairLinkedThroughBayFeatures(cfg) {
  return LegacyHakoImportAndHealing.healLinkedThroughBay(cfg);
}

function stripLegacyWallBuckets(cfg) {
...
function convertLegacyWallBucketsToWallFeaturesOnce(cfg) {
  if (!cfg || typeof cfg !== 'object') return cfg;
  ensureWallFeatureMap(cfg);
...
function ensurePrintedMaterialDefinition(cfg) {
...
function upgradeConfigToCurrentStorage(cfg) {
  if (!cfg || typeof cfg !== 'object') return cfg;
...
function migrateLegacyWallFeatures(cfg) {
