...
function insetTrianglePerpendicular(p1, p2, p3, d) {
 ...
 */
function generateFinkTruss(span, height, chordW) {
 ...
 */
function generateMonoPitchTruss(span, riseAtX0, riseAtXL, height, chordW) {
  const L = span;
 ...
 */
function generateParallelChordTruss(span, height, chordW) {
  const L = span, h = height;
 ...
 */
function generateTrussGeometry(roofStyle, span, spanDims, chordW) {
 ...
 */
function trussGeometryToPart(geom, partId, partName, materialKey) {
...
function layoutCropContextIsActive(cropCtx) {
  return !!(cropCtx && cropCtx.rootCfg && layoutCutsActive(cropCtx.rootCfg));
}

function layoutCropToWorld(cropCtx, p) {
  if (cropCtx && typeof cropCtx.toWorld === 'function') return cropCtx.toWorld(p);
  return { x: p.x, y: p.y };
}

function clippedBlockFootprintForLayoutCrop(blockCfg, cropCtx) {
...
function cleanPolygonVertices(poly, eps) {
  eps = eps || 0.001;
  const out = [];
  for (const p of (poly || [])) {
...
function clipPolygonByXMin(poly, xMin) {
  if (!poly || poly.length < 3) return [];
  const out = [];
...
function clipPolygonByXMax(poly, xMax) {
  if (!poly || poly.length < 3) return [];
  const out = [];
...
function clipPolygonToXRange(poly, xMin, xMax) {
  if (!poly || poly.length < 3) return [];
...
function clipPolygonByYMin(poly, yMin) {
  if (!poly || poly.length < 3) return [];
  const out = [];
...
function clipPolygonByYMax(poly, yMax) {
  if (!poly || poly.length < 3) return [];
  const out = [];
...
function trimGeneratedTrussGeometryToSpanInterval(geom, start, end) {
...
function trussSpanIntervalForLayoutCrop(blockCfg, axis, span, position, cropCtx, interiorFrame) {
...
function columnBearingInsideLayoutCrop(blockCfg, axis, wall, pos, cropCtx, matT, interiorFrame) {
  const clippedWorld = clippedBlockFootprintForLayoutCrop(blockCfg, cropCtx);
...
function generateTrussesForBlock(blockCfg, blockPlan, sfx, nameLbl, cropCtx) {
...
function resolveTrussAxis(blockCfg) {
  const t = blockCfg.trusses || {};
  const ridge = effectiveRidgeDir(blockCfg);
  const isGable = blockCfg.roofStyle === 'gabled' || blockCfg.roofStyle === 'parapet_gable';
  let axis = t.axis;
...
function trussInteriorFrame(blockCfg, blockPlan) {
...
function wallXBracingEnabled(t) {
  if (!t) return false;
 ...
 */
function squareCappedStripPolygon(x1, y1, x2, y2, width) {
...
function pointOnSegmentEps(p, a, b, eps) {
  eps = eps || 1e-6;
...
function pointOnPolygonBoundary(p, poly, eps) {
  for (let i = 0; i < poly.length; i++) {
...
function addUniquePoint(out, p, eps) {
  eps = eps || 1e-5;
...
function removeNearlyCollinearPoints(poly, eps) {
  eps = eps || 1e-5;
...
function unionTwoStarPolygons(polyA, polyB) {
  const pts = [];
...
function connectedXBraceOutline(w, h, memberW) {
...
function generateXBraceBayPart(bayW, wallH, memberW, id, name, note) {
