...
/* iPad Safari can pan the editor sideways when any fixed/flex ancestor has a
 * few pixels of horizontal overflow. Make the toolbox slightly wider, keep its
 * contents inside that width, and continuously clamp horizontal scroll while an
 * editor modal is open. The canvas area remains the only horizontal scroller. */
(function installEditorToolboxVerticalScrollOnlyCss() {
  ...
  const clampHorizontalScroll = () => {
    const modal = document.getElementById('openingEditorModal');
    const iwModal = document.getElementById('iwModal');
    const activeModals = [modal, iwModal].filter(el => el && getComputedStyle(el).display !== 'none');
    ...
    for (const m of activeModals) {
      els.push(m, m.querySelector('.oe-dialog'), m.querySelector('.oe-body'), m.querySelector('.oe-toolbox'));
    }
    for (const el of els) {
  ...
  document.addEventListener('scroll', clampHorizontalScroll, true);
  document.addEventListener('touchmove', clampHorizontalScroll, { passive: true, capture: true });
  document.addEventListener('touchend', clampHorizontalScroll, true);
  window.addEventListener('resize', clampHorizontalScroll);
  setInterval(clampHorizontalScroll, 250);
})();
