...
function measuredArcTargetInfo(cfg, cut) {
  cfg = cfg || CONFIG;
...
function resolveMeasuredArcCut(cut, cfg) {
...
function resolveLayoutCutGeometry(cut, cfg) {
  return resolveMeasuredArcCut(cut, cfg || CONFIG);
}

function updateMeasuredArcCutGeometry(cut, cfg) {
...
function refreshMeasuredLayoutCuts(cfg) {
...
function layoutLineSide(a, b, p) {
  return (b.x - a.x) * (p.y - a.y) - (b.y - a.y) * (p.x - a.x);
}

function lineSegmentInfiniteLineIntersection(p1, p2, a, b) {
  const dx = p2.x - p1.x, dy = p2.y - p1.y;
...
function clipPolygonByHalfPlane(poly, a, b, keepSide) {
...
function sampleLayoutCutSegments(cut, cfg) {
...
function clipPolygonByLayoutCuts(poly, cuts, cfg) {
...
function shouldLayoutClipPart(part) {
...
function wingLocalToWorldPointForLayoutCut(cfg, tr, p) {
  const face = tr.face;
...
function worldToWingLocalPointForLayoutCut(cfg, tr, p) {
  const face = tr.face;
  const off = Number(tr.offset) || 0;
  const W = Number(tr.mainW != null ? tr.mainW : (cfg && cfg.width)) || 0;
  const D = Number(tr.mainD != null ? tr.mainD : (cfg && cfg.depth)) || 0;
...
function layoutCutTransformForPart(part, cfg) {
  cfg = cfg || CONFIG;
...
function layoutCutWingIndexFromPart(part) {
  const id = (part && part.id) || '';
  const m = id.match(/_wing(\d+)/);
  return m ? parseInt(m[1], 10) : null;
}

function layoutCutWallFromPart(part) {
  const id = (part && part.id) || '';
...
function layoutCutPartWallKind(part) {
  const id = (part && part.id) || '';
...
function layoutCutWallClipSpec(part, cfg) {
...
function segmentParamIntersections(a, b, c, d) {
  const out = [];
...
function uniqueSortedParams(vals) {
...
function keptIntervalsOnSegmentInsidePolygon(a, b, poly) {
  if (!poly || poly.length < 3) return [];
  const params = [0, 1];
  for (let i = 0; i < poly.length; i++) {
...
function clipLocalPolygonToXInterval(poly, x0, x1) {
  let out = (poly || []).map(p => ({ x: p.x, y: p.y }));
  if (out.length < 3) return [];
  // Keep x >= x0
...
function clipRectToXInterval(r, x0, x1) {
...
function clipLineToXInterval(l, x0, x1) {
...
function applyLayoutCutsToWallPart(part, cuts, cfg) {
...
function applyLayoutCutsToPart(part, cuts, cfg) {
...
function applyLayoutCutsToGeneratedParts(cfg, parts) {
...
function layoutCutRetainedXIntervalForPart(part, cfg) {
...
function clipOpeningSpecsForLayoutCut(part, specs, cfg, minW = 1.0) {
...
function tallyDoorSpecs(doorSpecs) {
  const t = {};
...
function slotCenterInOneOfRanges(slot, ranges) {
  const c = (slot.start + slot.end) / 2;
  return (ranges || []).some(r => c >= r[0] - 0.01 && c <= r[1] + 0.01);
}

function halfWidthSlotSegment(slot, forWingPiece = false) {
...
function applyFloorSplitHalfWidthSlots(slots, ranges) {
...
function floorSplitRangesByFace(cfg, matT, sideLen) {
...
function sortedSlotsByStart(slots, reverse) {
  const a = [...(slots || [])];
  a.sort(reverse ? ((x, y) => y.start - x.start) : ((x, y) => x.start - y.start));
  return a;
}

function floorSlotDepth(slot, matT) {
  return matT * ((slot && slot.depthFactor) || 1);
}

function wingOuterDepth(wing) {
  return Math.max(0, Number(wing && wing.depth) || 0);
}

function wingSideWallBodyLength(wing, matT) {
...
function mainFloorSeamSlotsForWing(cfg, plan, wing) {
...
function buildPanelizedWingFloorPart(cfg, plan, wing, index) {
...
function chooseFloorWingSplits(cfg, plan, wings) {
...
function generateMergedFloor(cfg, plan) {
...
function floorOutlineOpeningRectFor(cfg, x, y, w, h, opts = {}) {
...
function floorOutlineOpeningPathsFor(cfg, x, y, w, h, opts = {}) {
...
function translatedHolePathsForRect(holes, rx, ry, rw, rh, cfg) {
  const out = [];
...
function embeddedRailSlotPathLocal(z, widthAcrossTrack) {
...
function embeddedRailGuideLinesLocal(z, widthAcrossTrack, type = 'etch') {
  const o = embeddedRailOrientation(z);
  if (o === 'ew') {
    const cy = z.h / 2 + (z.trackOffset || 0);
    return [
...
function generateEmbeddedRailParts(cfg, plan) {
...
function generateInterFloorPanels(cfg, plan) {
