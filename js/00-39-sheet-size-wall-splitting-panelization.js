...
function rectLooksLikePartOuterBoundary(part, r) {
...
function pathBoundsForDiscardCheck(d) {
  try {
...
function pathLooksLikePartOuterBoundary(part, p) {
...
function partRoleUsesDisposableCutColor(part) {
...
function partIsFixtureOrDetailLike(part) {
  normalizePartMetadata(part);
...
function partIsWindowOrDoorFrameLike(part) {
  normalizePartMetadata(part);
  const role = String((part && part.meta && part.meta.role) || '').toLowerCase();
  const id = String((part && part.id) || '').toLowerCase();
  const name = String((part && part.name) || '').toLowerCase();
...
function partMayHaveGreenDiscardInteriors(part) {
  return partRoleUsesDisposableCutColor(part) || partIsWindowOrDoorFrameLike(part);
}

function cutOpExplicitlyDiscarded(op) {
  if (!op) return false;
...
function cutOpExplicitlyStructural(op) {
  if (!op) return false;
  const t = String(op.type || '').toLowerCase();
...
function boundsStrictlyContain(outer, inner, tol = 0.05) {
...
function cutOpIsInsideAnotherCut(part, op, kind, index) {
...
function sheetSplitCandidateRole(part) {
  normalizePartMetadata(part);
...
function sheetSplitNeedsSplicePlates(role) {
  return ['core_wall', 'floor_panel', 'interfloor_panel', 'roof_panel'].includes(role);
}

function sheetSplitCount(lengthMm, maxMm, minSegMm) {
...
function sheetSplitBoundaries(lengthMm, maxMm, minSegMm, staggerMm = 0) {
...
function sheetSplitFeatureRectsForAxis(part, axis) {
...
function sheetSplitBoundaryHitsFeature(boundary, rects, axis, pad = 1.25) {
...
function sheetSplitAdjustedBoundaryCandidate(raw, rect, axis, prev, next, minSeg, maxSeg) {
  const pad = 1.25;
...
function sheetSplitAvoidFeatureBoundaries(bounds, part, axis, minSeg, maxSeg) {
...
function svgPathLooksClosed(d) {
  return /[Zz]\s*$/.test(String(d || '').trim());
}

function translatePathPointsToLocal(pts, x0, y0) {
  return (pts || []).map(p => ({ x: p.x - x0, y: p.y - y0 }));
}

function clippedSegmentPathsToBox(x1, y1, x2, y2, x0, y0, x1b, y1b) {
  const boxPoly = [
    { x: x0,  y: y0  },
    { x: x1b, y: y0  },
    { x: x1b, y: y1b },
...
function sheetSplitClipPartGeometryToBox(part, x0, x1, y0, y1, cfg) {
...
function makeSheetSplitSegmentPart(part, col, row, x0Local, x1Local, y0Local, y1Local, totalCols, totalRows, cfg) {
  const minX = -(part.bboxOffsetX || 0);
...
function makeSheetSplitSplicePlate(part, splitAtLocal, index, cfg, maxAlong = null, axis = 'x') {
  normalizePartMetadata(part);
...
function splitPartForSheet(part, cfg) {
  normalizePartMetadata(part);
...
function applySheetSplittingToParts(parts, cfg) {
