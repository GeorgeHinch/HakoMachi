...
function configHeightIncludesParapet(cfg) {
  return !!(cfg && cfg.heightIncludesParapet);
}

function wallBodyHeightFromConfig(cfg) {
  if (!cfg) return 1;
...
function migrateHeightToIncludeParapet(cfg) {
...
function groundFloorHeightFromFormOrConfig(stdFloorH) {
...
function derivedFloorHeightFromAbsolute(totalHeightInclParapet, floors, parapetH, groundHOverrideOrNull) {
...
function totalHeightFromFloors(floors, floorH, parapetH, groundHOverrideOrNull) {
  const n = Math.max(1, Math.round(Number(floors) || 1));
...
function syncHeightFloorModeFields({ writeComputed = true } = {}) {
 ...
 */
function pruneOpeningsBeyondWall(cfg) {
...
function applyBuildingTypeDefaults() {
...
function effectiveMat(partId, defaultMat) {
  return MaterialRegistry.partMaterial(partId, defaultMat, CONFIG);
}

function isKnownMat(matId) {
...
function renderPartsOutput(parts) {
  lastParts = parts;
...
function showMaterialPicker(partId, anchor) {
