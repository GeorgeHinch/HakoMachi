...
let iwPanning      = false;
let iwPanStartPx   = null;  // {x, y} in canvas pixels at mousedown
let iwPanStartOff  = null;  // {x, y} = iwOffX/iwOffY at mousedown
let iwSpaceDown    = false;
...
let iwShieldBoundsDrag = null;
let iwShieldBoundsStart = null;
...
let iwRailResizeDrag = null;
let iwRailResizeStart = null;
...
let iwSelection = [];
let iwDragMoving = null;
let iwClipboard  = [];

const IW_ROOF        = 'roof'; // sentinel value for the roof tab
const IW_TAB_SPACING = 35;     // mm between wall tongue tabs
...
function iwIsSelected(type, idx) {
...
function iwSelect(type, idx, additive) {
  if (additive) {
...
function iwApplyDragDelta(dx, dy, origPositions, bW, bD) {
...
function iwClampRailTrackOffset(z) {
  if (!z) return;
...
function iwRailTrackAcrossInfo(z) {
  const pr = embeddedRailProfile(z);
  const band = (pr.sleeperWidthMm || 16) + 2 * (pr.safetyMarginMm || 2);
...
function iwApplyRailTrackDrag(curMM) {
...
function iwApplyRailZoneResize(curMM) {
...
function iwRailResizeCursor(handle) {
...
function iwPrimaryPointer(e) {
  return !e || e.button == null || e.button === 0;
}

function iwBindPointerAndMouse(el, handler) {
 ...
 */
function iwHandleObjectGestureMouseDown(e) {
...
function iwDistributeSelected(axis) {
...
function iwApply() { iwClose(); regenerate(); }
 ...
 */
function iwShieldAddOpening(side) {
 ...
 * false otherwise.
 */
function iwShieldwallEdgeClick(e) {
...
function iwFloorIdxForPart(partId) {
 ...
 */
function migrateEquipCountsToItems() {
 ...
 */
function iwAutoSeedRoofEquipment() {
  const plan = (iwWingIndex != null && iwWingPlan) ? iwWingPlan : lastPlan;
  if (!plan) return 0;
  const tgt = iwTarget();
  if (!tgt.rooftopItems) tgt.rooftopItems = [];
...
const IW_INTER = 'inter'; // sentinel for inter-floors tab

function iwBuildTabs() {
...
function iwGrid()  { return Math.max(1, Number(iwGridValue) || 5); }
function iwSnap(v) { const g = iwGrid(); return Math.round(v / g) * g; }
...
function iwSnapEdgeAware(v, min, max) {
  min = Number(min) || 0;
  max = Number(max);
...
function iwSnapAxis(v, max) {
  return iwSnapEdgeAware(v, 0, max);
}
function iwSnapPt(x, y) {
  const { bW, bD } = iwBounds();
...
function iwTrussColumnPreviewEtches() {
...
function iwMainFloorConnectedRects() {
...
function iwClamp(pt, bW, bD) { return { x: Math.max(0, Math.min(bW, pt.x)), y: Math.max(0, Math.min(bD, pt.y)) }; }

function iwAxisAlign(start, end) {
...
function iwHandleMouseLeave() { if (!iwDragItem) { iwCurMM = null; if (!iwDrag) iwRender(); } }


/* ---- Tab computation with stagger & half-lap detection ---- */
function iwComputeTabs(wi, walls, matT) {
  const w = walls[wi];
  const isH = Math.abs(w.y2 - w.y1) < 0.01;
  const len = isH ? Math.abs(w.x2 - w.x1) : Math.abs(w.y2 - w.y1);
...
function generatedBuildingName(cfg) {
...
function verifyTongueSlotMatch(parts, matT) {
