...
function inferPartWingIndex(part) {
  const meta = (part && part.meta) || {};
...
function normalizePartMetadata(part, extra = {}) {
  if (!part) return part;
...
function normalizePartsMetadata(parts) {
  for (const p of (parts || [])) normalizePartMetadata(p);
  return parts || [];
}

function applyPartMeta(part, meta = {}) {
  return normalizePartMetadata(part, meta);
}

function exportPartScopeInfo(part) {
...
function exportPartWallInfo(part) {
  normalizePartMetadata(part);
...
function exportPartZipPath(part, folderForMat, usedPaths) {
  const scope = exportPartScopeInfo(part);
  const wall = exportPartWallInfo(part);
