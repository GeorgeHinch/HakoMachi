...
function oeGetAutoOpenings(wall) {
  const plan = oeActivePlan();
  if (!plan) return [];
...
function oeExposureZoneVisual(zone) {
...
function oeExposureZoneOverlaySvg(cfg, plan, wall, ox, oy, s, wallW, wallH) {
 ...
 */
function renderBuildingFootprintMap(svgEl, highlightWall) {
  if (!svgEl) return;
...
function layoutReferencePhysicalOffset(face, edgeLen, pos, span) {
...
function collectLayoutReferenceOpeningsForBlock(blockCfg, blockPlan, bounds) {
  const out = [];
...
function generateBuildingLayoutReferencePart(cfg, mainPlan) {
  if (!cfg) return null;
 ...
 */
function oeIsTouchLikeEditor() {
  return oeLastPointerType === 'touch' || oeLastPointerType === 'pen' ||
         (window.matchMedia && window.matchMedia('(pointer: coarse)').matches);
}

function oeBeginDocumentDrag(sourceEvent, onMove, onUp, options = {}) {
...
function oeTouchPointerShouldStartEditorGesture(e) {
...
function oeCanvasPointerDownImpl(e) {
...
function oeEventTargetOpeningIndex(e) {
  const t = e && e.target;
 ...
 */
function oeBayOppositeWall(sourceWall) {
  return sourceWall === 'front' ? 'back'
       : sourceWall === 'back'  ? 'front'
       : null;
}

function oeEnsureWallFeatureWorkingArray(face) {
...
function oeCommitWorkingWallFeatures(face) {
  const target = oeEditTarget();
...
function oeWallFeatureEditorOps(face) {
  const target = oeEditTarget();
...
function oeApplyEmbeddedRailBayMove(op, sourceWall) {
 ...
 */
function oeSyncLinkedBayPartner(op, sourceWall) {
 ...
 */
function oeDeleteLinkedBayPartner(op, sourceWall) {
  if (op && op._embeddedRail) return false;
...
function oeCanvasMouseDownImpl(e) {
...
function oeDeleteSelectedItems() {
  if (oeSelectedSet.size === 0) return;
...
function oeShowEditorToast(message) {
...
function oeGetWinDims(styleKey) {
...
function oeGetDoorDims(styleKey) {
...
function oeGetAwningDims(styleKey) {
...
function oeGetBalconyDims(styleKey) {
...
function oeGetFixtureDims(styleKey) {
...
function oeGetPrintedItemDims(styleKey) {
...
function oeGetBayDims(styleKey) {
...
function oeDefaultOverrideCladdingStyle() {
