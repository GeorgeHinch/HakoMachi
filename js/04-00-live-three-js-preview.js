...
function materialDefinitionForId(cfg, materialId) {
  return MaterialRegistry.get(materialId, cfg || CONFIG, false);
}

function materialColourHex(cfg, materialId, fallback = '#cccccc') {
  return MaterialRegistry.colorHex(materialId, cfg || CONFIG, fallback);
}

function materialColourNumber(cfg, materialId, fallback = 0xcccccc) {
  return MaterialRegistry.colorNumber(materialId, cfg || CONFIG, fallback);
}

function claddingMaterialIdForStyle(cfg, styleKey) {
  return MaterialRegistry.claddingMaterialId(styleKey, cfg || CONFIG);
}

function claddingColourHexForStyle(cfg, styleKey, fallback = '#cbbfa3') {
  return MaterialRegistry.claddingColorHex(styleKey, cfg || CONFIG, fallback);
}

function shadeHexColour(hex, factor = 0.75) {
...
function material3DForId(cfg, materialId, opts = {}, fallback = 0xcccccc) {
  return MaterialRegistry.threeMaterial(materialId, cfg || CONFIG, opts, fallback);
}
...
const _printedItemTextureCache3D = new Map();

function printedItemSvgMarkup3D(styleKey, w, h) {
...
function printedItemTexture3D(styleKey, w, h) {
...
function buildWallTexture(cfg, wallW, wallH, baseH, openings, flipX, styleKeyOverride) {
 ...
 */
function get3DWallProfileForFace(cfg, plan, face) {
  const H = plan.H;
...
function doorOpeningAs3DBayDoor(op, fallbackStyle) {
...
function effective3DWindowStyleKey(cfg, op, isGround) {
...
function get3DWallOpenings(cfg, plan, face) {
  if (!plan) return [];
 ...
 * the face. */
function getBlankedWindows3D(cfg, face) {
 ...
 */
function buildWallMesh3D(wallW, wallH, baseH, thickness, openings, material, mirrorX, texture, gablePitch, slantPitch) {
 ...
 */
function addWindowGlass3D(group, wallW, wallH, wallZPos, wallRot, openings, glassMat, mirrorX) {
  for (const op of openings) {
    if (op.type !== 'window') continue;
 ...
 */
function build3DEquipMesh(item) {
...
function triListToThreeGeometry(tris, opts = {}) {
...
function stlTriListToMesh(tris, material, opts = {}) {
...
function buildGeneratedStlPreviewGroup(cfg) {
  const group = new THREE.Group();
...
// addEventListener + e.preventDefault() does NOT work for this case.
