...
export function downspoutHasScupperBox(cfg, face, styleKey) {
...
export function downspoutScupperLocalSpec(totalW, totalH) {
...
export function downspoutScupperRoofLineY(cfg, face) {
...
export function downspoutScupperWallHoleY(cfg, face, fixtureY, sc) {
...
export function downspoutScupperWallBoxY(cfg, face, fixtureY, sc) {
...
export function effectiveFixturePieces(cfg, face, styleKey, instW, instH, forceScupper = null) {
...
export function downspoutRooflineTopY(cfg, plan, face, styleKey) {
...
export function scaleFixtureFeatureForPiece(feature, sx, sy) {
  const f = { ...(feature || {}) };
...
export function scaleFixtureFeaturesForPiece(features, naturalW, naturalH, actualW, actualH) {
...
export function generateFixtureParts(cfg, fixtureGroups) {
...
export function fixtureThroughHoleSpec(style, fW, fH) {
...
export function addFixtureThroughHoleGeometry(rects, paths, x, y, w, h, spec) {
  if (!spec) return;
  const hx = x + spec.x;
  const hy = y + spec.y;
  const hw = spec.w;
...
export function addFixtureThroughHolesToWall(rects, paths, cfg, face, xMirror) {
...
export function addFixtureThroughHolesToCladding(rects, paths, cfg, face, xMirror, shiftX = 0, shiftY = 0, opts = {}) {
  const features = wallFeaturesForFace(cfg, face);
  for (const f of (features || [])) {
    if (!f || f.type !== 'fixture') continue;
    const style = FIXTURE_STYLES[f.style];
...
export function addDownspoutScupperThroughHolesToWall(rects, paths, cfg, face, xMirror) {
  if (!cfg) return;
  const features = wallFeaturesForFace(cfg, face);
  for (const f of (features || [])) {
...
export function addDownspoutScupperThroughHolesToCladding(rects, paths, cfg, face, xMirror, shiftX = 0, shiftY = 0) {
  if (!cfg) return;
  const features = wallFeaturesForFace(cfg, face);
  for (const f of (features || [])) {
    if (!f || f.type !== 'fixture' || f.style !== 'downspout') continue;
...
export function addDownspoutScupperMarksToCladding(lines, cfg, face, xMirror, shiftX = 0, shiftY = 0) {
  if (!cfg) return;
  const features = wallFeaturesForFace(cfg, face);
  for (const f of (features || [])) {
    if (!f || f.type !== 'fixture' || f.style !== 'downspout') continue;
