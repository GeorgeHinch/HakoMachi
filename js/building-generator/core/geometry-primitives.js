 ...
 */
export function placeTongues(edgeLength, tongueWidth, minMargin, forbidden, targetCount, minSpacing) {
...
export function mirrorSlotsAlongEdge(slots, edgeLength) {
  return (slots || [])
    .map(s => {
 ...
 */
export function isFeatureCenteredOnEdge(op, edgeLength, eps = 0.25) {
  if (!op || op.x == null || op.w == null || !edgeLength) return true;
  const c = Number(op.x) + Number(op.w) / 2;
  return Math.abs(c - edgeLength / 2) <= eps;
}

export function wallFaceHasOrientationSensitiveFeatures(cfg, face, edgeLength) {
...
export function wallAssemblyKeyOffset(cfg, face, edgeLength, matT, tw) {
...
export function shiftedTonguesAreSafe(tongues, edgeLength, forbidden, margin) {
  const fz = forbidden || [];
...
export function applyWallAssemblyKeyToTongues(tongues, edgeLength, cfg, face, forbidden, matT, tw, marginOverride) {
 ...
 */
export function buildWallPath(spec) {
...
*/
export function buildLShapePathWithFeatures(mainW, mainH, extW, extH, extRight, opts) {
...
*/
export function buildTShapePathWithFeatures(mainW, mainH, leftCW, leftCH, rightCW, rightCH, opts) {
  const matT  = opts.matT;
  const td    = matT;
  const fx    = v => v.toFixed(3);
...
/* Build a simple L-shaped perimeter path (no edge tongues — connections added as
   ...
   stepY = mainH − extH (where the inner step starts from the top). */
export function buildLShapePath(mainW, mainH, extW, extH, extRight) {
  const stepY = mainH - extH;
  const totalW = mainW + extW;
  if (extRight) {
...
export function isGableEndWall(cfg, which) {
 ...
 *
 * Returns 'ew' or 'ns'. */
export function effectiveRidgeDir(cfg) {
...
export function wallHasParapet(cfg, which) {
 ...
 */
export function buildGableWallPath(wallW, wallH, pitch, spec, slopeTabSpec = null) {
 ...
 */
export function buildSlantedWallPath(wallW, wallH, pitchLeft, pitchRight, spec) {
  const { matT, kerf } = spec;
  const k  = kerf || 0;
  const td = matT;
  const tk = -k;
...
export function buildPanelPerimeterPath(spec) {
  const { width: W, height: H, matT, kerf } = spec;
  const k = kerf || 0;
  const td = matT;
  const tk = -k;
...
export function clipVerticalLineByCuts(x, y0, y1, cuts, margin) {
...
export function clipHorizontalLineByCuts(y, x0, x1, cuts, margin) {
  margin = (margin == null) ? 0.5 : margin;
  let blocked = [];
  for (const c of cuts) {
...
export function generateCladdingPattern(style, panelLeft, panelTop, panelRight, panelBottom, cutOpenings, topEdge) {
  const out = [];
...
export function slotPath(x, y, w, h, kerf) {
  const k = kerf || 0;
