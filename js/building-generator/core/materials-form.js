...
export function sanitiseFolderName(str) {
...
export function materialLibraryPayload() {
...
export function normaliseImportedMaterialLibrary(data) {
  return MaterialRegistry.normalizeProfilePayload(data);
}

export function applyMaterialLibrary(library) {
...
export function materialProfileIdFromName(name) {
  const data = window.HakoMachiGithubDataShared;
  if (data && typeof data.slugify === 'function') return data.slugify(name, 'material-profile');
  return String(name || 'material-profile').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'material-profile';
}

export function saveCurrentMaterialProfileToBrowser() {
...
export async function saveCurrentMaterialProfileToGithub() {
...
export function applyBrowserMaterialProfile(profileId) {
...
export function applyPendingMaterialProfileHandoff() {
...
export function exportMaterialLibrary() {
...
export function openMaterialLibraryImportPicker() {
...
export function initMaterialsForm() {
  renderMaterialsForm();
}

export function renderMaterialsForm() {
