...
export function generateParapetRoof(cfg, plan) {
...
export function generateFlatRoof(cfg, plan) {
  const { H, matT, tw, parapetH, roofStyle, sideLen, fbWidth } = plan;
...
export function generateFlatOverhangRoof(cfg, plan) {
  const { H, matT, tw, parapetH, roofStyle, sideLen, fbWidth } = plan;
...
export function generateSlantedRoof(cfg, plan) {
  const { H, matT, tw, parapetH, roofStyle, sideLen, fbWidth } = plan;
...
export function gableRoofAngleLockStations(cfg, plan, perpLen, ohVerge) {
...
export function generateGableRoofAngleLockOutline(edgeLen, roofCos, roofSin, tabAlong, tabDepth, tabCenterFromApex, webDepth = null) {
...
export function pointsToPath(pts, ox = 0, oy = 0) {
  return pts.map((p, i) => `${i === 0 ? 'M' : 'L'} ${(ox + p.x).toFixed(3)},${(oy + p.y).toFixed(3)}`).join(' ') + ' Z';
}

export function generateGableRoofAngleLockParts(cfg, plan, stationCount, halfSpan, pitch, ridgeDir, slotSpec = null) {
...
export function generateGabledRoof(cfg, plan) {
  const { H, matT, tw, parapetH, roofStyle, sideLen, fbWidth } = plan;
 ...
 */
export function generateGabledRoofCladding(perpLen, slantLen, halfSpan, pitch, matT, styleSpec, ridgeDir) {
...
export function generateRoofCladdingPanel(id, name, panelW, panelH, styleSpec, cfg) {
  if (!styleSpec) return null;
...
export function generateFloor(cfg, plan) {
...
export function generateMergedFloorRaw(cfg, plan) {
...
export const LASER_SAFETY_EDGE_BUFFER_MM = 6.35; // 1/4 in on every sheet edge
