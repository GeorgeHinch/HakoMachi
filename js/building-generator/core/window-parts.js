 ...
 * no frame layers.
 */
export function generateBlankedWindowPanels(cfg, upperCount, groundCount) {
  const parts = [];
...
export function groupWindowSpecsBySize(specs) {
...
export function winDimsFromSpec(spec) {
 ...
 * keeps working. */
export function generateWindowPartsFromSpecs(cfg, openingSpecs) {
 ...
 * core hole. */
export function generateBlankedWindowPanelsFromSpecs(cfg, blankedSpecs) {
...
export function generateWindowParts(cfg, totalWindows, groundFloorWindowCount) {
...
export function generateWindowPartsForDims(cfg, winDims, count, suffix) {
  if (count <= 0) return [];
  const cT = cfg.claddingThickness;
...
export function suffixId(suffix) {
  if (!suffix) return '';
...
export function generateDoorInserts(cfg, doorTallies) {
...
export function cropDoorStyleForSpec(baseDoor, spec) {
...
export function groupDoorSpecsBySize(specs) {
  const groups = new Map();
  for (const s of (specs || [])) {
...
export function generateDoorInsertsFromSpecs(cfg, doorSpecs) {
