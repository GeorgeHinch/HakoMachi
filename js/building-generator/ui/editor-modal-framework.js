...
export function editorTopbarKit(bar) {
  return {
    sep() {
...
export function editorNormalizeRefs(refs) {
...
export function editorAlignRefs(refs, mode) {
...
export function editorDistributeRefs(refs, axis, mode = 'gap') {
  refs = editorNormalizeRefs(refs);
...
export function editorAddAlignmentGroup(bar, opts) {
...
export function editorAddClipboardGroup(bar, opts) {
  const kit = editorTopbarKit(bar);
  const parent = opts.parent || bar;
...
export function editorAddNumberControl(bar, opts) {
...
export function editorAddGridControls(bar, opts) {
  const kit = editorTopbarKit(bar);
...
export function editorIsTypingTarget(target) {
  if (!target) return false;
  const tag = (target.tagName || '').toUpperCase();
  return tag === 'INPUT' || tag === 'TEXTAREA' || tag === 'SELECT' || target.isContentEditable;
}

export function editorModalIsOpen(modalId) {
  const modal = document.getElementById(modalId);
  return !!(modal && modal.style.display !== 'none');
}

export function editorHandleCommonKeyDown(e, opts) {
...
export function editorHandleCommonKeyUp(e, opts) {
  if (!editorModalIsOpen(opts.modalId)) return false;
...
export function alignIconSvg(type) {
  const W = 18, H = 14;
  let rects = '';
  switch (type) {
    case 'left':
...
export function distributeIconSvg(dir) {
...
export function oeSelectedEditorRefs() {
...
export function oeAlignMode(mode) {
  if (editorAlignRefs(oeSelectedEditorRefs(), mode)) {
    oeCommit();
    oeRender();
  }
}
export function oeAlignLeft()    { oeAlignMode('left'); }
export function oeAlignRight()   { oeAlignMode('right'); }
export function oeAlignCenterH() { oeAlignMode('centerX'); }
export function oeAlignTop()     { oeAlignMode('top'); }
export function oeAlignBottom()  { oeAlignMode('bottom'); }
export function oeAlignMiddleV() { oeAlignMode('centerY'); }

/* ---- Distribute functions ---- */

export function oeDistributeHorizontal() {
...
export function oeDistributeVertical() {
