...
export function applyIntersectionOverride(intersection, overrides = {}) {
  ...
  next.crosswalks = override.crosswalksEnabled
    ? (intersection.crosswalks || []).filter(feature => {
        const armOverride = armOverrideFor(override, feature.roadId, feature.endpoint);
        return armOverride?.enabled !== false && armOverride?.crosswalkEnabled !== false;
  ...
  next.stopBars = override.stopBarsEnabled
    ? (intersection.stopBars || []).filter(feature => {
        const armOverride = armOverrideFor(override, feature.roadId, feature.endpoint);
        return armOverride?.enabled !== false && armOverride?.stopBarEnabled !== false;
  ...
  next.tactilePavers = override.tactilePaversEnabled
    ? (intersection.tactilePavers || []).filter(feature => {
        const armOverride = armOverrideFor(override, feature.roadId, feature.endpoint);
        return armOverride?.enabled !== false && armOverride?.tactilePaversEnabled !== false;
      })
    : [];
  next.curbReturns = override.curbGuidesEnabled
    ? (intersection.curbReturns || []).filter(feature => {
        const armOverride = armOverrideFor(override, feature.fromRoadId, feature.fromEndpoint);
        return armOverride?.enabled !== false && armOverride?.curbGuideEnabled !== false;
      })
    : [];
  return next;
}
