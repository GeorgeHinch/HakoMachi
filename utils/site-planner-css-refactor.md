...
- `site-planner-shared.css` — Site Planner-specific adapter: grid shell, top bar, toolbar, canvas, sidebar, bottom status bar, map/card selection states, tool flyouts, empty image overlay, and responsive planner layout.
...
Keep planner-only geometry/layout classes in `site-planner-shared.css`:
